...
$ripfile = "C:\\Program Files\\WindThunder\\HeroineAnthem\\Image\\WTImage.original";
//$ripfile = "C:\\Program Files\\WindThunder\\Heroine Anthem\\Ani\\WTAnime.bin";
...
$fo = fopen("C:\\Program Files\\WindThunder\\HeroineAnthem\\Image\\WTImage.bin", "w");
fputs($fo, str_pad("WT_Package_File", 24, chr(0), STR_PAD_RIGHT));
fputs($fo, str_pad(pack("V*", $count), 8, chr(0), STR_PAD_RIGHT));
$baseoff = (164*$count)+0x20;
for ($i=0; $i<$count; $i++) {
	fputs($fo, str_pad($name[$i], 24, chr(0), STR_PAD_RIGHT));
	fputs($fo, str_pad($source[$i], 128, chr(0), STR_PAD_RIGHT));
	//if(!file_exists("outgfx2\\".$name[$i])) die("OUCH!");
	$tempsize = filesize("C:\\Translations\\ha1\\outgfx2\\".trim($name[$i]));
	//echo $name[$i] . " $tempsize\n";
	fputs($fo, pack("V*", $tempsize));
	fputs($fo, str_pad(pack("V*", $baseoff), 8, chr(0), STR_PAD_RIGHT));
	$baseoff+=$tempsize;
}
...
for ($i=0; $i<$count; $i++) {
	print "Inserting $name[$i]... ";
	$fd = fopen("outgfx2\\".$name[$i], "rb");
	$fddump = fread($fd, filesize("outgfx2\\".$name[$i]));
	fputs($fo, $fddump);
	fclose($fd);
	print "done!\n";
}
